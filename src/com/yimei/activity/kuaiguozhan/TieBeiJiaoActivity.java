 ...
 */
public class TieBeiJiaoActivity extends Activity {
			...
			if (MyApplication.INTENT_ACTION_SCAN_RESULT.equals(intent
				...
				if (tag.equals("高温点亮作业员")) { // 作业员
					...
					if(gaowen_user.getText().toString().toUpperCase().trim().equals("")
						...
						ToastUtil.showToast(TieBeiJiaoActivity.this,"作业员不能为空",0);
				...
				if (tag.equals("高温点亮批次号")) { // 作业员
					...
					if(gaowen_sid1.getText().toString().toUpperCase().trim().equals("")
						...
						ToastUtil.showToast(TieBeiJiaoActivity.this,"批次号不能为空",0);
						MyApplication.nextEditFocus(gaowen_sid1);
						return;
					}
			...
			if (v.getId() == R.id.gaowen_user) {
				if (actionId == EditorInfo.IME_ACTION_DONE) { // 用户名
					if (gaowen_user.getText().toString().toUpperCase()
						...
						ToastUtil.showToast(TieBeiJiaoActivity.this,
								"用户名不能为空！", 0);
						return false;
					}
								...
								if (Integer.parseInt(jsonValue.get("bok")
									...
									map.put("zcno1",
											jsonValue.containsKey("zcno1") ? jsonValue
													.getString("zcno1") : MyApplication.DiDianLiu_ZCNO);
									mList.add(map);
